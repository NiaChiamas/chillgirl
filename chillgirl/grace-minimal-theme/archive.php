...
<div id="main-content">
	...
	<!-- page head section -->
	<?php if(get_theme_mod('archive_header_show') != ""){ ?>
		...
		<?php
			...
			if(get_theme_mod('archive_header_width') == "wide"){
				$page_header_class = "wide-header";
			}
			
			// get height of page header
			if(get_theme_mod('archive_header_height') != ""){
			
				// height set within customizer
				$page_header_height = get_theme_mod('archive_header_height');
			...
			// content text color
			$page_header_text_color = get_theme_mod('archive_page_header_content_color');
			
			// content background color
			if(get_theme_mod('archive_page_header_background') != ""){
				
				// background style class
				$content_background_class = "page-head-inside-background";
				
				// styling to add for background color + opacity
				$content_background_style = 'background-color:rgba(' . grace_hex_convert(get_theme_mod('archive_page_header_background_color')) . ',' . get_theme_mod('archive_page_header_background_opacity') . ');max-width:'. get_theme_mod('archive_page_header_content_max_width') . 'px';
		...
		<section class="page-header <?php echo esc_attr($page_header_class); ?>">
			<div class="container">
				...
				<!-- page header inner -->
				<div class="section-inner no-border" style="min-height:<?php echo esc_attr($page_header_height); ?>px;background-image:url('<?php echo esc_url(get_theme_mod('archive_image_upload')); ?>');color:<?php echo esc_attr($page_header_text_color); ?>;">
					<div class="page-head-inside <?php echo esc_attr($content_background_class); ?>" style="<?php echo esc_attr($content_background_style); ?>">
						<h1 class="font-montserrat-reg"><?php echo esc_html(get_theme_mod('archive_page_header_title', 'Archive')); ?></h1>
						<?php
							// Archive date
						...
						?>
						<p class="font-opensans-reg"><?php echo esc_attr($month . " " . $year); ?></p>
					</div>
				</div>
	...
	<?php 
		...
		$page_sidebar = get_theme_mod('archive_sidebar_position');
